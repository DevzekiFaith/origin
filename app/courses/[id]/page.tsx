...
export default function CourseDetailPage() {
  ...
  const readingCompanion = course ? getCompanionProductForCourse(course.id) : null;
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] text-[#121316] selection:bg-amber-400 selection:text-zinc-950 font-sans">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 space-y-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="space-y-4">
              ...
              <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight text-[#121316] leading-tight">
                {course.title}
              </h1>
            ...
            <div className="p-7 sm:p-8 rounded-3xl bg-[#FFFFFF] border border-[#E8E8E3] shadow-xs space-y-2">
              <div className="text-xs font-mono text-amber-700 uppercase tracking-wider font-bold">
            ...
            <div className="p-7 sm:p-8 rounded-3xl bg-[#FFFFFF] border border-[#E8E8E3] shadow-xs space-y-3">
              <div className="text-xs font-mono text-amber-700 uppercase tracking-wider font-bold">
                02 // WHAT WILL I UNDERSTAND?
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                {(course.outcomes || []).slice(0, 4).map((outcome, idx) => (
                  <div key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm text-[#3F3F46]">
            ...
            <div className="p-7 sm:p-8 rounded-3xl bg-[#FFFFFF] border border-[#E8E8E3] shadow-xs space-y-3">
              ...
              <div className="flex flex-wrap gap-2 pt-2 text-xs font-mono">
                {["01 SEE IT", "02 THINK", "03 CHOOSE", "04 DISCOVER", "05 TRY AGAIN", "06 USE IT", "07 REFLECT"].map((stage, idx) => (
                  <span key={idx} className="px-3 py-1.5 rounded-lg bg-[#F3F3EE] border border-[#E2E2DC] text-[#3F3F46] font-semibold">
                    {stage}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-[#FFFFFF] border border-[#E2E2DC] rounded-3xl p-7 sm:p-9 shadow-[0_10px_35px_rgba(0,0,0,0.04)] space-y-6">
              ...
              {readingCompanion && (
                <div
                  className={`p-4 sm:p-5 rounded-2xl border transition-all cursor-pointer ${
                    includeEbook ? "bg-amber-500/10 border-amber-500/30" : "bg-[#FAFAF8] border-[#E8E8E3]"
                  }`}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div className="text-xs text-[#3F3F46] space-y-1">
                      <span className="font-bold text-[#121316] block text-sm">
              ...
              {!isEnrolled && (
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-[#FAFAF8] hover:bg-[#F3F3EE] border border-[#E2E2DC] text-[#3F3F46] text-xs font-mono transition-colors flex items-center justify-center gap-2 cursor-pointer font-semibold"
                >
        ...
        {/* 14. GO DEEPER // ORIGIN READING COMPANION SECTION */}
        {readingCompanion && (
          <section className="p-8 sm:p-12 md:p-14 rounded-3xl bg-[#FFFFFF] border border-[#E2E2DC] shadow-sm relative overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
              <div className="md:col-span-4 flex justify-center">
                <div className="relative w-48 h-64 rounded-2xl overflow-hidden shadow-xl border border-[#E8E8E3] bg-zinc-900 group">
                  {readingCompanion.imageUrl ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center text-white bg-gradient-to-br from-amber-600 to-zinc-950">
                      <Book className="w-10 h-10 mb-2 text-amber-300" />
                      <span className="font-bold text-sm">{readingCompanion.name}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="md:col-span-8 space-y-4">
                <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#F3F3EE] border border-[#E2E2DC] text-xs font-mono font-bold uppercase text-amber-800">
                ...
                <div className="p-5 rounded-2xl bg-[#FAFAF8] border border-[#E8E8E3] space-y-1">
                  <div className="text-xs font-mono uppercase text-[#71717A] font-bold">
                ...
                <div className="pt-3 flex flex-wrap items-center gap-4">
                  <Link
                    ...
                    className="px-6 py-3 rounded-xl bg-[#121316] text-white font-mono text-xs font-bold hover:bg-amber-600 transition-colors flex items-center gap-2 shadow-sm cursor-pointer"
                  >
                    <span>READ THE COMPANION →</span>
                  </Link>
                  <span className="text-xs text-[#71717A] font-mono">
                    Available as instant digital PDF or browser interactive reading
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(course.outcomes || []).map((outcome, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-[#FAFAF8] border border-[#E8E8E3] flex items-start gap-3">
                ...
                <p className="text-sm text-[#27272A] font-medium">{outcome}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between border-b border-[#E8E8E3] pb-4">
            <div>
              <div className="text-xs font-mono text-amber-700 uppercase tracking-wider font-bold">EXPERIENTIAL ROADMAP</div>
              <h2 className="text-2xl font-bold text-[#121316]">Modules & Missions</h2>
            </div>
            <span className="text-xs font-mono text-[#71717A] font-semibold">{course.modules?.length || 6} Modules</span>
          </div>

          <div className="space-y-3">
            {(course.detailedModules || []).map((mod, idx) => (
              <div key={idx} className="p-6 sm:p-7 rounded-2xl bg-[#FFFFFF] border border-[#E8E8E3] shadow-sm space-y-2">
                <div className="flex items-center justify-between text-xs font-mono text-[#71717A]">
                  <span className="font-bold text-amber-700">MODULE 0{idx + 1}</span>
                  <span>{mod.estimatedTime || "30 mins"}</span>
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-[#121316]">{mod.title}</h3>
        ...
        <section className="p-8 sm:p-14 rounded-3xl bg-[#FFFFFF] border border-[#E2E2DC] text-center max-w-3xl mx-auto space-y-6 shadow-sm">
          ...
          <h2 className="text-2xl sm:text-4xl font-extrabold text-[#121316]">
